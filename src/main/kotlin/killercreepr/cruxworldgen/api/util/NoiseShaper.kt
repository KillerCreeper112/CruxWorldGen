...
open class NoiseShaper(
  ...
  companion object{
    ...
    fun dummy() : NoiseShaper  = dummy
  }

  class Dummy : NoiseShaper(listOf())
  ...
  fun interface ShapingFunction {
    companion object {
      ...
      val EPIC = ShapingFunction { n ->
        // Work in [-1..1]
        var v = n.coerceIn(-1.0, 1.0)

        // 1) Make negative range more "basin-like"
        // Push the middle down to create broad lowlands and valleys
        v = valleyBias(v, amount = 0.28, width = 0.80)

        // 2) Keep midrange flatter so you get plains / shelves
        v = flattenMiddle(v, width = 0.55, strength = 0.82)

        // 3) Add ridge character without going too spiky
        v = ridges(v, power = 1.45) // lower power = less needle-y

        // 4) Give mountains a chunky top (plateau only on the high end)
        v = plateau(v, top = 0.62, softness = 0.22)

        // 5) Only tiny terraces high up (optional)
        // (This creates interesting strata on peaks without turning everything into steps.)
        val high01 = smoothstep01(((v - 0.55) / (1.0 - 0.55)).coerceIn(0.0, 1.0))
        val terr = terrace(v, step = 0.12, blend = 0.65)
        v = v * (1.0 - 0.35 * high01) + terr * (0.35 * high01)

...
class SimplePropPointGrid(
  ...
  override fun pointsForChunk(ctx: GenerateContext, chunkX: Int, chunkZ: Int): List<PropPoint> {
    ...
    val chunkWorldX = chunkX * chunkWidth
    val chunkWorldZ = chunkZ * chunkDepth
    ...
    val endX = chunkWorldX + chunkWidth + spacingBlocks
    val endZ = chunkWorldZ + chunkDepth + spacingBlocks
    ...
    while (gridX <= endX) {
      ...
      while (gridZ <= endZ) {
        ...
        if (localX in 0..<chunkWidth && localZ in 0..<chunkDepth) {

...
class FlattenPadTerraformer(
  ...
  override fun terraformChunk(region: LimitedRegion, inst: StructureInstance, template: StructureTemplate) {
    ...
    if (anchorLX !in 0..<chunkWidth || anchorLZ !in 0..<chunkDepth) return
    ...
    for (lx in minX..maxX) {
      for (lz in minZ..maxZ) {
        ...
        val jitter =
          (hashSigned01(inst.seed xor (wx.toLong() * 341873128712L) xor (wz.toLong() * 132897987541L))) * jitterAmp
        ...
        if (surface > desiredSurface) {
          for (y in surface downTo desiredSurface + 1) {
            if (y in chunk.minHeight until chunk.maxHeight) {
              chunk.setBlock(lx, y, lz, BlockData.EMPTY)
            }
          }
          // Paint top
  ...
  private fun paintTopSoil(chunk: ChunkContext, x: Int, y: Int, z: Int) {
    if (y !in chunk.minHeight until chunk.maxHeight) return
    // super simple for now (later: biome palette)
    //todo chunk.setBlock(x, y, z, Material.GRASS_BLOCK)

  ...
  override fun run(s: TerraformState): Boolean {
    for (lx in s.workMinX..s.workMaxX) {
      for (lz in s.workMinZ..s.workMaxZ) {
        ...
        val rough01 = (s.ctx.noise.detail3D(wx, surface, wz) + 1.0) * 0.5
    ...
    for (y in (surface + 1)..desired) {
      ...
      val mat =
        if (depthFromTop < topSoil) {
          ...
          val r01 = (s.ctx.noise.detail3D(wx, y, wz) + 1.0) * 0.5
...
class TerraformState(
  ...
  val chunkWorldX: Int get() = ctx.chunkX * 16
  val chunkWorldZ: Int get() = ctx.chunkZ * 16
  ...
  // Height model
  var plane: Plane = Plane(0.0, 0.0, 64.0)
...
interface TerrainPalette {
  fun topBlock(ctx: GenerateContext, wx: Int, y: Int, wz: Int): Material
  fun fillerBlock(ctx: GenerateContext, wx: Int, y: Int, wz: Int): Material
  fun foundationBlock(ctx: GenerateContext, wx: Int, y: Int, wz: Int): Material
}

object DefaultTerrainPalette : TerrainPalette {
  override fun topBlock(ctx: GenerateContext, wx: Int, y: Int, wz: Int): Material = Material.GRASS_BLOCK
  override fun fillerBlock(ctx: GenerateContext, wx: Int, y: Int, wz: Int): Material = Material.DIRT
  override fun foundationBlock(ctx: GenerateContext, wx: Int, y: Int, wz: Int): Material = Material.STONE
}
